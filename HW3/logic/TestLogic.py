...
for ask in asks:
	print "\nAsking : " + str(ask)
	matches = kb.kb_ask(ask)
	if len(matches) == 0:
		print "No matches in KB"
	else:
		for match in kb.kb_ask(ask):
			print match,
		print
...
action = logic.Action(["color", "bigbox", "red"], logic.kb())
action.act_retract()

action = logic.Action(["size", "bigbox", "big"], logic.kb())
action.act_retract()
...
kb.kb_assert(["color", "bigbox", "red"])

kb.kb_assert(["size", "bigbox", "big"])

print "\033[0;32m\n=================== Testing against Why ===================\x1b[0m"


for fact in kb.facts:
	kb.kb_why(logic.Statement(fact.statement))
...
list_of_bindings = kb.kb_ask_plus(statement_list1)
...
list_of_bindings = kb.kb_ask_plus(statement_list2)
